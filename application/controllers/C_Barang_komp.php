...
class C_Barang_komp extends CI_Controller {

    function __construct() {
        parent::__construct();
		$this->load->model('Model_barang_komp');
		if(!isset($_SESSION)) {
			session_start();
	   }
    ...
    public function barang_komp() {
		$data = array(
			'title' => "DATA BARANG KOMPETITOR"
        );

        $data['barang'] = $this->Model_barang_komp->get_all()->result();
        
		$this->load->view('dist/barang_komp/barang_komp', $data);
	}


	public function tambah_barang_komp(){
		if (isset($_POST['simpan'])){
				$kode = $_POST['kode_barang'];
				$nama = $_POST['nama_barang'];

				$isi = array(
					'kode_barang_komp' => $kode,
					'nama_barang_komp' => $nama
				);
				$simpan = $this->Model_barang_komp->simpan_data($isi);
				if($simpan)
				{
					echo "<script>alert('Data Berhasil disimpan');</script>";
					header("Location: barang_komp");

				}
		}
		$data = array(
			'title' => "TAMBAH BARANG KOMPETITOR"
        );
		$this->load->view('dist/barang_komp/tambah_barang_komp',$data);
	}
	public function edit_barang_komp(){
		...
		if (isset($_POST['ubah'])){
			...
			$ubah = $this->Model_barang_komp->ubah_data($id_barang,$isi);
			header("Location: barang_komp");
			if($ubah){
				echo "<script>alert('Data Berhasil disimpan');</script>";
					header("Location: barang_komp");
		...
		$data = array(
			'title' => "EDIT BARANG KOMPETITOR"
		);
		$data['barang'] = $this->Model_barang_komp->get_one($id)->row_array();
		$this->load->view('dist/barang_komp/edit_barang_komp',$data);
	}

	public function hapus_barang_komp(){
		$id = $this->uri->segment(3);
		$where = array('id_barang_komp'=>$id);
		$hapus = $this->Model_barang_komp->hapus_data($where);
		redirect('C_barang_komp/barang_komp');
	}
