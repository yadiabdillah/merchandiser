...
class C_Md extends CI_Controller {

    function __construct() {
        parent::__construct();
		$this->load->model('Model_md');
		if(!isset($_SESSION)) {
			session_start();
	   }
    ...
    public function md() {
		$data = array(
			'title' => "DATA Merchandiser"
        );

        $data['md'] = $this->Model_md->get_all()->result();
        
		$this->load->view('dist/md/md', $data);
	}


	public function tambah_md(){
		if (isset($_POST['simpan'])){
				...
				$simpan = $this->Model_md->simpan_data($isi);
				if($simpan)
				{
		...
		$data = array(
			'title' => "TAMBAH MERCHANDISER"
        );
		$this->load->view('dist/md/tambah_md',$data);
	}
	public function edit_md(){
		...
		if (isset($_POST['ubah'])){
			...
			$ubah = $this->Model_md->ubah_data($id_md,$isi);
			header("Location: md");
			if($ubah){
				echo "<script>alert('Data Berhasil disimpan');</script>";
					header("Location: md");
		...
		$data['md'] = $this->Model_md->get_one($id)->row_array();
		$this->load->view('dist/md/edit_md',$data);
	}

	public function hapus_md(){
		$id = $this->uri->segment(3);
		$where = array('id_md'=>$id);
		$hapus = $this->Model_md->hapus_data($where);
		redirect('C_md/md');
	}
