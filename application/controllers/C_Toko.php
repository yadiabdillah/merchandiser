...
class C_Toko extends CI_Controller {
    ...
    public function toko() {
        ...
        $data['barang'] = $this->Model_toko->get_all()->result();
        
		$this->load->view('dist/toko/toko', $data);
	}
	...
	public function tambah_toko(){
		if (isset($_POST['simpan'])){
				...
				$simpan = $this->Model_toko->simpan_data($isi);
				if($simpan)
				{
		...
		$this->load->view('dist/toko/tambah_toko',$data);
	}
	public function edit_toko(){
		...
		if (isset($_POST['ubah'])){
			...
			$ubah = $this->Model_toko->ubah_data($id_toko,$isi);
			header("Location: toko");
			if($ubah){
				echo "<script>alert('Data Berhasil disimpan');</script>";
					header("Location: toko");
		...
		$data['toko'] = $this->Model_toko->get_one($id)->row_array();
		$this->load->view('dist/toko/edit_toko',$data);
	}

	public function hapus_toko(){
		$id = $this->uri->segment(3);
		$where = array('id_toko'=>$id);
		$hapus = $this->Model_toko->hapus_data($where);
		redirect('C_toko/toko');
	}
