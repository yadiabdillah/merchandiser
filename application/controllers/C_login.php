...
class C_login extends CI_Controller {
	...
	public function index() {
		...
		$this->load->view('dist/login', $data);
    }
    ...
    public function ceklogin() {
        ...
        $data = $this->Model_login->cek_login($username,$pass,$roles)->num_rows();
        
        if ($data > 0){
			$isi = $this->Model_login->cek_login($username,$pass,$roles)->row_array();
			if(!isset($_SESSION)) {
				session_start();
		   }
		
            $_SESSION['id'] = $isi['id_md'];
            $_SESSION['status'] = "login";
            $_SESSION['roles'] = $isi['roles'];
            $_SESSION['nama'] = $isi['nama_md'];
           /* $data_session = array(
				'id' => $isi['id_md'],
                'status' => "login",
                'roles' => $isi['roles'],
        ...
        $data['barang'] = $this->Model_login->get_all_barang()->result();
