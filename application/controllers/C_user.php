...
class C_User extends CI_Controller {
    ...
    public function user() {
        ...
        $data['barang'] = $this->Model_user->get_all()->result();
        
		$this->load->view('dist/user/user', $data);
	}
	}
	...
	public function tambah_user(){
		if (isset($_POST['simpan'])){
				...
				$simpan = $this->Model_user->simpan_data($isi);
				if($simpan)
				{
		...
		$this->load->view('dist/user/tambah_user',$data);
	}
	public function edit_user(){
		...
		if (isset($_POST['ubah'])){
			...
			$ubah = $this->Model_user->ubah_data($id_md,$isi);
			header("Location: user");
			if($ubah){
		...
		$data['user'] = $this->Model_user->get_one($id)->row_array();
		$this->load->view('dist/user/edit_user',$data);
	}

	public function hapus_user(){
		$id = $this->uri->segment(3);
		$where = array('id_md'=>$id);
		$hapus = $this->Model_user->hapus_data($where);
